...
//	}
//	
//	public int doOperate(String cmd, int f, int s) { //doOperate 호출
//
...
//		System.out.println("-".repeat(10));
//		
//		return mo.operate(f,  s); //숫자 두개 넘겨줘서 operate가 호출되면서 연산진행 //polymorphism(다형성)
//	}
//	
...
public class MyOperatorContainer {
	...
	public MyOperator getOperator(String cmd) {
		...
		switch (cmd) {
		case "+":
			mo = new MyOperatorForAdd();
			break;
		case "-":
			mo = new MyOperatorForSub();
			break;
		case "*":
			mo = new MyOperatorForMul();
			break;
		case "/":
			mo = new MyOperatorForDiv();
			break;
		default:
			mo = null;
			break;
